...
void GGrasshopperPlacerAgent::MoveAwayFromSiblings(GAgentGraphicsItem* pItem, int remainingItemsToBeAdded /*= 0*/ )
{
	if(!pItem)
		return;
...
{
	start();
// 	GAgentGraphicsItem* pAgIt = qobject_cast<GAgentGraphicsItem*>(sender());
// 	if(pAgIt) {
...
{
	foreach(GAgentGraphicsItem* pItem, ListItemsToBePlaced(0 /*all*/)) {
