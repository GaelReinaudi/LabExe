...
// 	connect(this, SIGNAL(NoMoreItemsToAddForNow(GAgentGraphicsItem*)), this, SLOT(start()));
}
...
GAgentGraphicsItem* GForcePlacerAgent::ProvideNewAgentGraphicsItem( GAgentGraphicsItem* pParentItem )
{
	GAgentGraphicsItem* pItem = GPlacerAgent::ProvideNewAgentGraphicsItem(pParentItem);
	return pItem;
}

void GForcePlacerAgent::AddItemToPlace( GAgentGraphicsItem* pItemToPlace, int remainingItemsToBeAdded /*= 0*/ )
{
...
// 	msleep(1000);
	...
	while(!AllForceZero) 
	{
		...
		foreach(GAgentGraphicsItem* pItem, ListItemsToBePlaced(0)) {
...
QVector2D GForcePlacerAgent::Force( GAgentGraphicsItem* pItem )
{
	QVector2D force;
	if(!pItem)
		return force;
	GAgentGraphicsItem* pParentItem = pItem->ParentAgentGraphicsItem();
	if(!pParentItem)
		return force;
	QSet<GAgentGraphicsItem*> siblings = pItem->SiblingAgentGraphicsItems();
	if(siblings.isEmpty())
		return  force;
	...
	foreach(GAgentGraphicsItem* pOtherItem, siblings) {
...
QVector2D GForcePlacerAgent::ShortestMoveForEdgeToEdge(GAgentGraphicsItem* pIt1ToMove, GAgentGraphicsItem* pIt2, double InflateRect1 /*= 0.0*/)
{
	QVector2D shortMove;
	if(!pIt1ToMove || !pIt2)
		return shortMove;
