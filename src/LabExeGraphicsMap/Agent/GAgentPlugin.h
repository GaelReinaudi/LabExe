...
*/
class LABEXEGRAPHICSMAP_EXPORT GAgentPlugin : public QObject
{
	Q_OBJECT
...
/*! \def G_REGISTER_PLUGIN_HARD_AGENT(ClassName)
\brief This is the Macro that makes your derived class from GAgent available to the Agent factory. 
...
*/
#define G_REGISTER_PLUGIN_HARD_AGENT(ClassName, UniqueIdForTheInstance)											\
G_1_PART_REGISTER_PLUGIN_HARD_AGENT(ClassName)																		\
	void RegisterAgentClassInAgentManager() const {																	\
	G_2_PART_REGISTER_PLUGIN_HARD_AGENT(ClassName, UniqueIdForTheInstance)											\
}																													

#define G_1_PART_REGISTER_PLUGIN_HARD_AGENT(ClassName)																\
...
#define G_2_PART_REGISTER_PLUGIN_HARD_AGENT(ClassName, UniqueIdForTheInstance)										\
	bool isOkkk = RegisterHardAgentInManager( # ClassName , CreatorUsedForTheAgentManager ## ClassName );			\
	AddToAgentShelf( CreatorUsedForTheAgentManager ## ClassName (UniqueIdForTheInstance, 0));
...
This macro is like G_REGISTER_PLUGIN_HARD_AGENT, but also register the GProgAgent in the e-Agent shelf
using RegisterProgAgentInManager();

The difference of those to macro is also justified by the fact that a GProgAgent 
